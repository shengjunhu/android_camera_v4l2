...
bool HwDecoder::init() {
    ...
    if (STATUS_PARAM > onStatus()) {
        ...
        //1.1-创建MediaCodec: OMX.rk.video_decoder.mjpeg、video/mjpeg
        if (mediaCodec == NULL) {
            mediaCodec = AMediaCodec_createDecoderByType(MIME_TYPE);
        }
        if (mediaCodec == NULL) {
            LOGE(TAG, "Not support for %s", "video/mjpeg")
        } else {
            //1.2-配置MediaCodec参数
...
uint8_t *HwDecoder::process(void *raw_buffer, size_t raw_size) {
    ...
    if (in_buffer_id < 0) {
        ...
        //当前解码是队列方式,第一帧输入,返回为空,后续输入返回上一帧
        //如果取当前帧要加do{}while(),取到输出返回,耗时30ms左右
